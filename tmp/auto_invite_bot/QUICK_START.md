...
**重要注意事项**：
1. **关闭系统Chrome**：运行程序前请关闭你正在使用的Chrome浏览器，避免数据冲突
2. **登录状态**：程序会使用你系统Chrome的登录状态（Cookie、token等）
3. **首次运行**：如果自动打开的浏览器未登录微信小店，需要手动登录一次
4. **登录持久化**：登录后自动保存，之后运行无需重复登录
5. **反检测**：程序使用多种技术模拟真人操作，难以被识别为自动化工具

**操作流程**：
```
关闭系统Chrome → 运行程序 → 自动打开Chrome（含登录状态）→ 开始邀约
```
...
### 问题3：程序运行时系统Chrome打不开

**原因**：程序正在使用系统的Chrome用户数据目录，冲突了

**解决**：
- 关闭正在运行的Selenium程序
- 等待几秒后即可正常打开系统Chrome
- 或者同时只运行其中一个（程序或手动使用Chrome）

### 问题4：自动打开的Chrome没有登录状态

**原因**：首次运行或登录已过期

**解决**：
1. 在自动打开的浏览器中手动登录微信小店账号
2. 登录成功后关闭程序
3. 再次运行程序，登录状态会自动保持
...
5. **关闭系统Chrome**：运行程序前请关闭系统Chrome，避免数据冲突
6. **网络稳定**：确保网络连接稳定，避免加载失败
7. **浏览器指纹**：程序使用真实Chrome浏览器和用户数据，难以被检测
